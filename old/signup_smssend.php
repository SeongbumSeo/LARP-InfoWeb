...
$query = $DB->query(sprintf("SELECT * FROM auth_data WHERE PhoneNumber = '%s' AND Passed = 1",$number));
if(!$query) {
...
$query = $DB->query(sprintf("INSERT INTO auth_data (PhoneNumber,Username,Code,Passed) VALUES ('%s','%s',%d,0)",
   $number,$name,$code));
if(!$query) {
	print("2|인증번호 발송에 실패했습니다.");
	exit;
