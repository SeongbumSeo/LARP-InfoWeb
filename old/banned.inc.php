<?php
$result = $DB->query("SELECT * FROM infoweb_ban_data WHERE IP = '".$_SERVER['REMOTE_ADDR']."' ORDER BY Until DESC LIMIT 1");
$BanData = $result->fetch_array();
$bancount = strtotime($data['Until'])-strtotime(GetDBServerTime($DB));
if($result->num_rows > 0 && $bancount > 0)
{
	session_unset();
	session_destroy();
