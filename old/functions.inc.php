<?php
require('classes/PHPMailer-5.2.14/PHPMailerAutoload.php');
...
function SendMail($to,$subject,$body) {
	...
	$mail->SMTPSecure = "ssl";
	$mail->SMTPAuth = true;
	$mail->Username = "larp.korea@daum.net";
	$mail->Password = "cygkutrmxpbgpzjq";

	$mail->CharSet = "utf-8";
	$mail->Encoding = "base64";

	$mail->From = "support@la-rp.co.kr";
	$mail->FromName = "LA:RP 서포트 센터";
	$mail->addAddress($to);
...
function InsertLog($userdata,$type,$contents,$succeed=null) {
	global $LOG;
	switch ($type) {
		case 'Admin':
			$sql = sprintf("
				INSERT INTO _log_infoweb_admin (UserID,Username,IP,Contents,Succeed) VALUES (%d,'%s','%s','%s',%d)",
				$userdata['ID'],$userdata['Username'],$_SERVER['REMOTE_ADDR'],
				$LOG->real_escape_string($contents),($succeed == null)? "NULL": (int)$succeed);
			break;
		case 'Faction':
			$sql = sprintf("INSERT INTO _log_infoweb_faction (UserID,Username,FactionID,IP,Contents) VALUES (%d,'%s',%d,'%s','%s')",
				$userdata['ID'],$userdata['Username'],$userdata['Faction'],$_SERVER['REMOTE_ADDR'],
				$LOG->real_escape_string($contents));
			break;
		default:
			$sql = sprintf("INSERT INTO _log_infoweb (UserID,Username,IP,Type,Contents) VALUES (%d,'%s','%s','%s','%s')",
				$userdata['ID'],$userdata['Username'],$_SERVER['REMOTE_ADDR'],
				$LOG->real_escape_string($type),
				$LOG->real_escape_string($contents));
			break;
	}
	$LOG->query($sql);
...
function GetFactionName($factionid,$resource) {
	$fresult = @$resource->query("SELECT Name FROM faction_data WHERE ID=".$factionid);
	if($fdata = @$fresult->fetch_row())
		return $fdata[0];
	return "미가입";
}
...
function GetJobName($jobid) {
	switch($jobid) {
		case 1:		$job = "청소부";			break;
		case 2:		$job = "차량정비사";			break;
		case 3:		$job = "총기상";			break;
		case 4:		$job = "마약상";			break;
		case 5:		$job = "농부";			break;
		case 6:		$job = "노점상";			break;
		case 7:		$job = "낚시꾼";			break;
		case 8:		$job = "피자배달부";			break;
		default:	$job = "무직";
	}
	return $job;
}

function AccessAdminKit($adminlevel,$limitedlevel,$showmessage=true)
{
...
function ConvertSecondsToTimeString($seconds) {
	...
	$s = (int)($seconds % 60);
	unset($string);
	if($d > 0) $string .= sprintf("%d일 ",$d);
	if($h > 0) $string .= sprintf("%d시간 ",$h);
	if($m > 0) $string .= sprintf("%d분 ",$m);
	if($s > 0) $string .= sprintf("%d초 ",$s);
	if(!strlen($string))
		$string = "시간 없음";
	else
...
function GetDBServerTime($resource) {
	return $resource->query("SELECT CURRENT_TIMESTAMP")->fetch_row()[0];
}

function GetSkinImage($skinid) {
	if($handle = @fopen("./skins/$skinid.png", "r"))
	{
		$skinimg = "./skins/$skinid.png";
		fclose($handle);
	}
	else
		$skinimg = "./skins/0.png";
...
function GetVehicleImage($vehiclemodel) {
	return "http://weedarr.wdfiles.com/local--files/veh/".$vehiclemodel.".png";
}
...
function GetLocationName($lx,$ly) {
	$gSAZones = array(
