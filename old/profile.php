...
/* 차량 */
$query = $DB->query("SELECT * FROM car_data WHERE OwnerType = 1 AND OwnerID = '".$UserData['ID']."'");
$NumCars = $query->num_rows;
for($c = 0; $CarData = $query->fetch_array(); $c++) {
	// 이미지
	...
	if($CarData['Towed'])
		...
		if($virtualworld >= 40000 && $virtualworld < 50000) {
			$plquery = $DB->query("SELECT Name FROM parkinglot_data WHERE ID = ".($virtualworld-40000));
			if($plquery->num_rows < 1)
				$CarLocation[$c] = "추적불가";
			else
				$CarLocation[$c] = $plquery->fetch_row()[0];
		}
...
$query = $DB->query(sprintf("
    ...
    ",$UserData['ID']));
$NumItems = $query->num_rows;
for($i = 0; $ItemData = $query->fetch_array(); $i++) {
	$type = $ItemData['Name'];
	$input = $ItemData['Data'];
	...
	*/
	...
	if(!strcmp($type,"차 키")) {
		if(strpos($input,'+') === strlen($input))
			...
			$cquery = $DB->query(sprintf("SELECT NumberPlate FROM car_data WHERE ID='%s'",intval($input)));
			$str_hidden = ($cquery->num_rows < 1)? "INVALID": $cquery->fetch_row()[0];
		}
	}
	// 2
...
$NumUserLogs = $query->num_rows;
for($i = 0; $UserLogDataTmp = $query->fetch_array(); $i++)
	$UserLogData[$i] = array(
...
$NumUserLogs += $query->num_rows;
for($i; $UserLogDataTmp = $query->fetch_array(); $i++) {
	unset($exploded);
	unset($value);
