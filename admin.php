...
if(AccessAdminKit($UserData['Admin'],5)) {
	if($_GET['func'] == "setpw") {
		...
		$setpwresult = mysql_query("UPDATE user_data SET Password = SHA1('".$password."') WHERE Username = '".$username."'",$DB);
...
if(AccessAdminKit($UserData['Admin'],3)) {
	if($_GET['func'] == "idban" && strlen($_POST['idban_username']) > 0) {
		...
		$bansql .= "1)";
		$banresult = mysql_query($bansql,$DB);
...
<?php
	if($_GET['func'] == "checkuserlog")
	{
		...
		$userlogresult = mysql_query($userlogsql,$LOG);
...
if(AccessAdminKit($UserData['Admin'],7)) {
	if($_GET['func'] == "loginsession") {
		...
		$loginsessionresult = mysql_query("SELECT Password,Username FROM user_data WHERE Username='".$dest."'",$DB);
