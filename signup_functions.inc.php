...
function CheckUserName($name,$resource) {
	...
	*/
	...
	// 6
	$query = mysql_query("SELECT * FROM user_data WHERE Username = '$name'",$resource);
	$available[6] = (mysql_num_rows($query) === 0);
	// 7
	if(mysql_num_rows(GetSignedAccountsByIP($_SERVER['REMOTE_ADDR'],$resource)) > 0) {
		$available[7] = false;
		$query = mysql_query("SELECT Username FROM multiaccount_data WHERE IP = '$_SERVER[REMOTE_ADDR]'",$resource);
