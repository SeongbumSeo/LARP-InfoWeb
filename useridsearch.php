...
<?php
	if(strlen($_GET['userid_name']) > 0) {
		$useridresult = @mysql_query("SELECT Username, ID FROM user_data WHERE Username LIKE '".mysql_real_escape_string(htmlspecialchars_decode($_GET['userid_name']))."'", $MySQL);
		for($i = 0; $useriddata[$i] = @mysql_fetch_row($useridresult); $i++) {
?>
