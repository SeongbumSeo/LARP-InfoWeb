...
function InsertLog($userdata,$type,$contents,$succeed=null) {
	...
	switch ($type) {
		case 'Admin':
			$sql = sprintf("
				INSERT INTO log_infoweb_admin (UserID,Username,IP,Contents,Succeed) VALUES (%d,'%s','%s','%s',%s)",
				$userdata['ID'],$userdata['Username'],$_SERVER['REMOTE_ADDR'],
				mysql_real_escape_string($contents),($succeed == null)? "NULL": (int)$succeed);
			break;
		case 'Faction':
			$sql = sprintf("INSERT INTO log_infoweb_faction (UserID,Username,FactionID,IP,Contents) VALUES (%d,'%s',%d,'%s','%s')",
				$userdata['ID'],$userdata['Username'],$userdata['Faction'],$_SERVER['REMOTE_ADDR'],
				mysql_real_escape_string($contents));
			break;
		default:
			$sql = sprintf("INSERT INTO log_infoweb (UserID,Username,IP,Type,Contents) VALUES (%d,'%s','%s','%s','%s')",
				$userdata['ID'],$userdata['Username'],$_SERVER['REMOTE_ADDR'],
...
function GetFactionName($factionid,$resource) {
	$fresult = @mysql_query("SELECT Name FROM faction_data WHERE ID=".$factionid,$resource);
	if($fdata = @mysql_fetch_row($fresult))
		return $fdata[0];
	return "미가입";
}
...
function GetDBServerTime($resource) {
	return mysql_result(mysql_query("SELECT CURRENT_TIMESTAMP",$resource),0);
}
