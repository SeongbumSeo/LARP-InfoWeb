...
switch($_GET['func']) {
	...
	case 'offlineprocess':
		if(AccessAdminKit($UserData['Admin'], 5)) {
			$name = str_replace('/', ',', mysql_real_escape_string($_GET['username']));
			...
			$query = mysql_query(sprintf("SELECT ID, Username FROM user_data WHERE Username = '%s' OR ID = %d", $name, (int)$name), $DB);
			if(mysql_num_rows($query) < 1) {
				InsertLog($UserData, "Admin", sprintf("오프라인 프로세스: NULL/%s/%s", $name, $logdata), false);
				printf("<b>%s</b> 계정을 찾을 수 없습니다.", $name);
				exit;
			}
			$destid = mysql_result($query, 0, 0);
			$destname = mysql_result($query, 0, 1);
			$query = mysql_query(sprintf("
				INSERT INTO offline_process (UserID, Confirmed, Contents, Function, Admin, Time) VALUES (%d, 0, '%s', '%s, %s', '%s', CURRENT_TIMESTAMP)", 
				$destid, mysql_real_escape_string($_GET['reason']), mysql_real_escape_string($_GET['process']), 
				(int)$_GET['value'], mysql_real_escape_string($adminname)), $DB);
	...
	case 'idunban':
		if(AccessAdminKit($UserData['Admin'], 5)) {
			if($_GET['data'] == "idunban") {
				mysql_query("UPDATE ban_data SET Valid = 0, ReleaseReason = '".$adminname.": ".mysql_real_escape_string($_POST['idunban_reason'])."' WHERE ID = ".(int)$_GET['dest']." AND IDBan = 1", $DB);
				$unbanresult = mysql_query("SELECT * FROM ban_data WHERE IDBan = 1 AND Valid = 0 AND ID = ".(int)$_GET['dest'], $DB);
			...
			{
				$dest = mysql_real_escape_string($_POST['idunban_dest']);
...
<?php
			if($_GET['data'] == "idbansearch")
				$unbanresult = mysql_query("SELECT * FROM ban_data WHERE Username LIKE '".$dest."' AND IDBan = 1 ORDER BY Time DESC", $DB);
			else
				$unbanresult = mysql_query("SELECT * FROM ban_data WHERE IDBan = 1 ORDER BY Time DESC LIMIT 100", $DB);
				//$unbanresult = mysql_query("SELECT * FROM ban_data WHERE IDBan = 1 AND (Until > CURRENT_TIMESTAMP OR Time > date_add(now(), INTERVAL -1 MONTH)) ORDER BY Time DESC", $DB);
	...
	case 'ipunban':
		if(AccessAdminKit($UserData['Admin'], 5)) {
			if($_GET['data'] == "ipunban") {
				mysql_query("UPDATE ban_data SET Valid = 0, ReleaseReason = '".$adminname.": ".mysql_real_escape_string($_POST['ipunban_reason'])."' WHERE ID = ".(int)$_GET['dest']." AND IDBan = 0", $DB);
				$unbanresult = mysql_query("SELECT * FROM ban_data WHERE IDBan = 0 AND Valid = 0 AND ID = ".(int)$_GET['dest'], $DB);
				if($unbandata = mysql_fetch_array($unbanresult))
				...
				$dest = mysql_real_escape_string($_POST['ipunban_dest']);
...
<?php
			if($_GET['data'] == "ipbansearch")
				$unbanresult = mysql_query("SELECT * FROM ban_data WHERE (Username LIKE '".$dest."' OR IP LIKE '".$dest."') AND IDBan = 0 ORDER BY Time DESC", $DB);
			else
				$unbanresult = mysql_query("SELECT * FROM ban_data WHERE IDBan = 0 ORDER BY Time DESC LIMIT 100", $DB);
				//$unbanresult = mysql_query("SELECT * FROM ban_data WHERE IDBan = 0 AND (Until > CURRENT_TIMESTAMP OR Time > date_add(now(), INTERVAL -1 MONTH)) ORDER BY Time DESC", $DB);
			for($i = 0; $unbandata[$i] = mysql_fetch_array($unbanresult); $i++) {
?>
						<tr>
							<td><?=$unbandata[$i]['Username']?></td>
...
<?php
    ...
    case 'killlog':
        if(AccessAdminKit($UserData['Admin'],3)) {
            $type = mysql_real_escape_string($_GET['data']);
            $dest = mysql_real_escape_string($_GET['dest']);
            ...
            $query = mysql_query(sprintf("SELECT ID, Username FROM user_data WHERE ID = %d OR Username = '%s'", (int)$dest, $dest), $DB);
...
errHandler:
            ...
            while($log = mysql_fetch_array($query)) {
                $spquery = mysql_query(sprintf("SELECT Username FROM user_data WHERE ID = %d",
                    (!strcmp($type, "user")) ? $log['KillerID'] : $log['UserID']), $DB);
                $numrows = mysql_num_rows($spquery);
                $unknown = "<span style=\"color: #AA0000;\">UNKNOWN</span>";
                if(!strcmp($type, "user"))
                    $killername = ($numrows) ? mysql_result($spquery, 0) : $unknown;
                else
                    $username = ($numrows) ? mysql_result($spquery, 0) : $unknown;
                printf("
                    <tr>
                        <td>%s</td>
                        <td>%s</td>
	...
	case 'carblowlog':
		if(AccessAdminKit($UserData['Admin'], 3)) {
			...
			if(!strcmp($_GET['data'], "vehicle")) {
				$vehicle = mysql_real_escape_string($_GET['vehicle']);
				$query = mysql_query(sprintf("SELECT ID, NumberPlate FROM car_data WHERE ID = %d OR NumberPlate = '%s'", 
			  		(int)$vehicle, str_replace("LA", "LA ", str_replace("LA ", "LA", $vehicle))), $DB);
				if(mysql_num_rows($query) < 1) {
				...
				$vehicle = mysql_result($query, 0, 0);
				$vehiclenumber = mysql_result($query, 0, 1);
				
				InsertLog($UserData, "Admin", sprintf("차량 블로우 로그 조회(차량): %s", $vehicle), true);
				$query = mysql_query(sprintf("SELECT * FROM log_carblow WHERE CarID = %d ORDER BY Time DESC", $vehicle), $LOG);
			}
			else if(!strcmp($_GET['data'], "user")) {
				$query = mysql_query(sprintf("SELECT ID, Username FROM user_data WHERE ID = %d OR Username = '%s'", 
					(int)$_GET['user'], mysql_real_escape_string($_GET['user'])), $DB);
				if(mysql_num_rows($query) < 1) {
				...
				$query = mysql_query(sprintf("SELECT * FROM log_carblow WHERE UserID = %d ORDER BY Time DESC", $user), $LOG);
			}
            else exit;
			if(isset($query)) {
				...
				while($log = mysql_fetch_array($query)) {
					printf("<tr><td>%d</td><td>%s</td><td>%d</td><td colspan=\"2\">%s</td></tr> <tr><td colspan=\"2\">%s</td><td>%s</td><td>%s</td></tr>",
						$log['CarID'],
						isset($vehiclenumber)? $vehiclenumber: mysql_result(mysql_query(sprintf("SELECT NumberPlate FROM car_data WHERE ID = %d", $log['CarID']), $DB), 0),
						$log['UserID'],
						isset($username)? $username: mysql_result(mysql_query(sprintf("SELECT Username FROM user_data WHERE ID = %d", $log['UserID']), $DB), 0),
	...
	case 'connectiplog':
		if(AccessAdminKit($UserData['Admin'], 3)) {
			...
			$query = mysql_query("SELECT * FROM log_connect WHERE Username LIKE '".$name."' OR IP LIKE '".$name."' ORDER BY Time DESC", $LOG);
	...
	case 'changenamelog':
		if(AccessAdminKit($UserData['Admin'], 3)) {
			$keyword = mysql_real_escape_string($_GET['name']);
			...
			if(strlen($keyword) == 0) {
				...
				$userquery = mysql_query(sprintf("SELECT ID, Username FROM user_data WHERE Username LIKE '%s' OR ID = %d", $keyword, (int)$keyword), $DB);
				if(mysql_num_rows($userquery) < 1) {
					InsertLog($UserData, "Admin", sprintf("개명 로그 조회(특정 유저): %s", $keyword), false);
					printf("<b>%s</b> 키워드에 해당하는 계정을 찾을 수 없습니다.", $keyword);
					exit;
				}
				$query = mysql_query(sprintf("SELECT * FROM log_user WHERE Type=5 AND UserID = %d ORDER BY Time DESC", mysql_result($userquery, 0, 0)), $LOG);
				InsertLog($UserData, "Admin", sprintf("개명 로그 조회(특정 유저): %s", mysql_result($userquery, 0, 1)), true);
			}
			else if(strpos($keyword, "Admin:") === 0) {
				$keyword = str_replace("Admin:", NULL, $keyword);
				$query = mysql_query(sprintf("SELECT * FROM log_user WHERE Type=5 AND Contents LIKE '%%(담당:%%%s%%)' ORDER BY Time DESC", $keyword), $LOG);
				InsertLog($UserData, "Admin", sprintf("개명 로그 조회(담당자): %s", $keyword), true);
			}
			else {
				$query = mysql_query(sprintf("SELECT * FROM log_user WHERE Type=5 AND Contents LIKE '%%%s%%' ORDER BY Time DESC", $keyword), $LOG);
	...
	case 'checkuserdata':
		if(AccessAdminKit($UserData['Admin'], 6)) {
			...
			$fquery = mysql_query("SHOW COLUMNS FROM user_data", $DB);
			if(stripos($name, ':') > 0) {
				$inparr = explode(':', $name, 2);
				$uquery = mysql_query("SELECT * FROM user_data WHERE ".$inparr[0]." LIKE '".$inparr[1]."'", $DB);
			}
			else
				$uquery = mysql_query("SELECT * FROM user_data WHERE Username LIKE '".$name."' OR ID LIKE '".$name."'", $DB);
			if(mysql_num_rows($uquery) < 1) {
				printf("<b>%s</b> 키워드에 해당하는 계정을 찾을 수 없습니다.", $name);
				exit;
			}
			else if(mysql_num_rows($uquery) > 1) {
				printf("<b>%s</b> 키워드에 해당하는 계정이 %d개 있습니다.<br />", $name, mysql_num_rows($uquery));
	...
	/*case 'databaseview':
		if($UserData['ID'] == ACCOUNT_ACU || $UserData['ID'] == ACCOUNT_WLABYR) {
			...
			while($dbdata = mysql_fetch_row($query)) {
				print("<tr>");
	...
	/*case 'deleteaccount':
		if(AccessAdminKit($UserData['Admin'], 6)) {
			...
			$query = mysql_query("SELECT ID, Username FROM user_data WHERE Username = '".$name."' OR ID = '".$name."'", $DB);
			if(mysql_num_rows($query) < 1) {
			...
			$name = mysql_result($query, 0, 1);
			$query = mysql_query("DELETE FROM user_data WHERE ID=".$id, $DB);
	...
	case 'processlog':
		if(AccessAdminKit($UserData['Admin'], 6)) {
			...
			$sql = "
				SELECT *
				FROM log_admin_types
				JOIN log_admin ON log_admin.Type = log_admin_types.Type
				WHERE log_admin.Type >= 20 AND log_admin.Type <= 24";
			$type = ($data == "user")? "담당자": "대상자";
			if(($data == "user" || $data == "dest") && strlen($name) > 0) {
				$udquery = mysql_query(sprintf("
					SELECT ID FROM user_data WHERE ID = '%d' OR Username LIKE '%s' OR AdminName LIKE '%s'", 
					intval($name), $name, $name), $DB);
				if(mysql_num_rows($udquery) < 1) {
                    if(strlen($name))
					   InsertLog($UserData, "Admin", sprintf("프로세스 로그 조회(%s): %s", $type, $name), false);
					printf("<b>%s</b> 키워드에 해당하는 %s 계정을 찾을 수 없습니다.", $name, $type);
					break;
				}
				$sql .= sprintf(" AND %s = %d", ($data == "user") ? "UserID" : "DestID", mysql_result($udquery, 0));
			}
			$sql .= "
				ORDER BY Time DESC";
			$limit = sprintf(" LIMIT %d, %d", $page*$pagesize, $pagesize);
			$aquery = mysql_query($sql.$limit, $LOG);
			
            if(strlen($name))
                InsertLog($UserData, "Admin", sprintf("프로세스 로그 조회(%s): %s", $type, $name), true);
			printf("%d", mysql_num_rows(mysql_query($sql, $LOG))/$pagesize);
			while($adata = mysql_fetch_array($aquery)) {
				$uquery = mysql_query("SELECT AdminName FROM user_data WHERE ID=".$adata['UserID'], $DB);
				$dquery = mysql_query("SELECT Username FROM user_data WHERE ID=".$adata['DestID'], $DB);
				// Type, AdminName, Username, Reason
				printf("|%s/%s/%s/%s", 
					$adata['Name'], str_replace('/', ',', mysql_fetch_row($uquery)[0]), mysql_fetch_row($dquery)[0], 
